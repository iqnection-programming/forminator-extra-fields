 ...
 * @since 1.0
 */
class Forminator_Extra_Dropzone_Upload extends Forminator_Upload {
	...
	public function __construct() {
		...
		$this->name = __( 'Dropzone File Upload', 'forminator' );
	}
...
//	}
	...
	public function markup( $field, $settings = array() ) {
		...
		$markup = parent::markup($field, $settings);

		$dropzoneConfig = ['x' => 1];

		// inject the dropzone config into the <input /> HTML
		$markup = preg_replace('/\<input[^\>]+id=\"([a-zA-Z0-9_\-])\"/','id="$1" data-dropzone="'.htmlentities(json_encode($dropzoneConfig)).'" ', $markup);

		$this->dropzone_includes();
		...
		return apply_filters( 'forminator_extra_dropzone_field_file_markup', $html, $field );
	}

	protected function dropzone_includes() {
		$config = [];

		wp_enqueue_script('forminator-extra-dropzone', forminator_extra_plugin_url().'vendor/dropzone/js/dropzone.min.js');
		wp_enqueue_style('forminator-extra-dropzone', forminator_extra_plugin_url().'vendor/dropzone/css/dropzone.min.css');
		wp_add_inline_script('forminator-extra-dropzone-global-config', '<script>Dropzone.autoDiscover = false;</script>');
	}
...
//					'forminator_extra_dropzone_upload_field_required_validation_message',
...
//		$rules .= '},' . "\n";
//
//		return apply_filters( 'forminator_extra_dropzone_field_file_validation_rules', $rules, $id, $field );
//	}
...
//				 * @param array $field
//				 */
//				$mime_types     = apply_filters( 'forminator_extra_dropzone_upload_field_mime_types', $mime_types, $field );
